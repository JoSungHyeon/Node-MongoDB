...
const upload = multer({
  storage: multerS3({
    s3: s3,
    bucket: 'sunghyeonforum1',
    key: function (req, file, cb) {
      const extension = path.extname(file.originalname); // 파일의 확장자 추출
      cb(null, `${Date.now()}${extension}`); // 파일 이름에 확장자 추가
    }
  })
});
...
app.post('/add', upload.single('img1'), async (req, res) => {
  ...
  // console.log(req.file.location) // 아마존에 저장된 이미지 url

  try {
    if(req.body.title == '') {
      res.send("제목을 입력해 주세요.");
    } else {
      await db.collection('post').insertOne({title: req.body.title, content: req.body.content, img1: req.file.location});
      res.redirect('/list/1');
    }
  } catch(e) {
      console.log(e);
